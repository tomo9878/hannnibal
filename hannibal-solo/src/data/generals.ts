...
function buildScenario1(): BoardPiece[] {
  ...
  const generals: BoardPiece[] = [
    // ── 初期配置済み将軍 ──────────────────────────────────────────
    { id: 'hannibal',  type: 'General', x: nc.x,  y: nc.y,  imagePath: `${BASE}images/tkn-gnrl-Hannibal.png`,  label: 'Hannibal',  strength: 7 },
    { id: 'hasdrubal', type: 'General', x: nc.x,  y: nc.y,  imagePath: `${BASE}images/tkn-gnrl-Hasdrubal.png`, label: 'Hasdrubal', strength: 3 },
    { id: 'hanno',     type: 'General', x: crt.x, y: crt.y, imagePath: `${BASE}images/tkn-gnrl-Hanno.png`,     label: 'Hanno',     strength: 2 },
    { id: 'p-scipio',  type: 'General', x: mas.x, y: mas.y, imagePath: `${BASE}images/tkn-gnrl-P. Scipio.png`, label: 'P. Scipio', strength: 4 },
    { id: 't-longus',  type: 'General', x: lil.x, y: lil.y, imagePath: `${BASE}images/tkn-gnrl-T. Longus.png`, label: 'T. Longus', strength: 4 },
    // ── ローマ予備将軍（Rest Position に待機） ────────────────────
    { id: 'fabius',            type: 'General', ...restPos('Fabius'),            imagePath: `${BASE}images/tkn-gnrl-Fabius.png`,            label: 'Fabius',            strength: 0 },
    { id: 'flaminius',         type: 'General', ...restPos('Flaminius'),         imagePath: `${BASE}images/tkn-gnrl-Flaminius.png`,         label: 'Flaminius',         strength: 0 },
    { id: 'marcellus',         type: 'General', ...restPos('Marcellus'),         imagePath: `${BASE}images/tkn-gnrl-Marcellus.png`,         label: 'Marcellus',         strength: 0 },
    { id: 'g-nero',            type: 'General', ...restPos('G. Nero'),           imagePath: `${BASE}images/tkn-gnrl-G. Nero.png`,           label: 'G. Nero',           strength: 0 },
    { id: 'a-paulus',          type: 'General', ...restPos('A. Paulus'),         imagePath: `${BASE}images/tkn-gnrl-A. Paulus.png`,         label: 'A. Paulus',         strength: 0 },
    { id: 'varro',             type: 'General', ...restPos('Varro'),             imagePath: `${BASE}images/tkn-gnrl-Varro.png`,             label: 'Varro',             strength: 0 },
    { id: 'scipio-africanus',  type: 'General', ...restPos('Scipio Africanus'),  imagePath: `${BASE}images/tkn-gnrl-Scipio Africanus.png`,  label: 'Scipio Africanus',  strength: 0 },
    // ── カルタゴ予備将軍（Rest Position に待機） ──────────────────
    { id: 'h-gisbo',  type: 'General', ...restPos('H. Gisbo'), imagePath: `${BASE}images/tkn-gnrl-H. Gisbo.png`, label: 'H. Gisbo', strength: 0 },
    { id: 'mago',     type: 'General', ...restPos('Mago'),     imagePath: `${BASE}images/tkn-gnrl-Mago.png`,     label: 'Mago',     strength: 0 },
  ]
  ...
  for (const city of allCities) {
    ...
    if (ROME_PC_PROVINCES.has(prov)) {
      pcs.push({ id: `pc-r-${city.name}`, type: 'PC', x: city.x, y: city.y, imagePath: `${BASE}images/tkn-PC-RomePC.png` })
    } else if (CARTHAGE_PC_PROVINCES.has(prov)) {
      pcs.push({ id: `pc-c-${city.name}`, type: 'PC', x: city.x, y: city.y, imagePath: `${BASE}images/tkn-PC-CarthPC.png` })
    }
  }
